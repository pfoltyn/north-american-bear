...
public class GA_SetupWizard : EditorWindow
{
	...
	void OnGUI ()
	{
		...
		case 0:
			...
			EditorGUILayout.Space();
			EditorGUILayout.LabelField("This Setup Wizard will help you set up GameAnalytics, and teach you how to track system events, crashes, custom events, etc.", EditorStyles.wordWrappedLabel);
			EditorGUILayout.Space();
		...
		case 6:
			...
			EditorGUILayout.BeginHorizontal();
			EditorGUILayout.SelectableLabel("GA.API.Design.NewEvent(\"Kill:Robot\");", EditorStyles.textField, new GUILayoutOption[] { GUILayout.Height(19) });
			if (GUILayout.Button(_copyCode, GUILayout.MaxWidth(82)))
			{
				TextEditor te = new TextEditor();
				te.content = new GUIContent("GA.API.Design.NewEvent(\"Kill:Robot\");");
				te.SelectAll();
				te.Copy();
			}
			EditorGUILayout.EndHorizontal();
